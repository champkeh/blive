import {info, warn, error, debug, printDanmaku} from './logger.js'
import DanmakuSocketApp from "./SocketCore.js"
import {extend} from "./utils.js"

export default class WebPlayerSocket extends EventTarget {
    static checkOptions(options) {
        if (options) {
            if (options.hostList && !Array.isArray(options.hostList)) {
                error("check", "WebPlayerSocket Initialize options hostList must be array.")
                return false
            }
            if (options.rid && options.token) {
                return true
            } else {
                error("check", "WebPlayerSocket Initialize options rid or token missing.")
                return false
            }
        } else {
    ...
    async initialize() {
        this.socket = new DanmakuSocketApp({
            rid: this.options.rid,
            uid: this.options.uid || 0,
            url: "wss://broadcastlv.chat.bilibili.com:443/sub",
            urlList: this.options.hostList.map(h => `wss://${h.host}:${h.wss_port}/sub`),
            retry: true,
            retryMaxCount: this.options.hostList.length,
            protover: 3,
            customAuthParam: [
                {
                    type: "string",
            ...
            onOpen: () => {
                const event = new CustomEvent('open', {detail: this.options})
                this.dispatchEvent(event)
            },
            onClose: () => {
                const event = new CustomEvent('close', {detail: this.options})
                this.dispatchEvent(event)
            },
            onError: (err) => {
            ...
            onReceivedMessage: (data) => {
                const event = new CustomEvent(data.cmd, {detail: data})
                this.dispatchEvent(event)
            },
            ...
            onHeartBeatReply: (data) => {
                const event = new CustomEvent('heartbeatreply', {detail: data})
                this.dispatchEvent(event)
            },
