import {debug, info, onLogger} from './logger.js'
...
class SocketCore {
    ...
    onOpen() {
        if (this.options.debug) {
            debug('internal', 'onOpen executing')
        }
    ...
    onMessage(msg) {
        try {
            ...
            if (this.options.debug) {
                debug('internal', 'onMessage executing')
    ...
    onClose() {
        if (this.options.debug) {
            debug('internal', 'onClose executing')
        }
    ...
    onError(err) {
        if (this.options.debug) {
            debug('internal', 'onError executing: ' + JSON.stringify(err))
        }
